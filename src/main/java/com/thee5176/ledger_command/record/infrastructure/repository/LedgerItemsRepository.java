package com.thee5176.ledger_command.record.infrastructure.repository;
...
public class LedgerItemsRepository {
    ...
    public void createLedgerItems(LedgerItems ledgerItems) {
        try {
            dslContext
                    .insertInto(Tables.LEDGER_ITEMS, Tables.LEDGER_ITEMS.ID, Tables.LEDGER_ITEMS.LEDGER_ID,
                            Tables.LEDGER_ITEMS.COA, Tables.LEDGER_ITEMS.AMOUNT, Tables.LEDGER_ITEMS.TYPE,
                            Tables.LEDGER_ITEMS.CREATED_AT, Tables.LEDGER_ITEMS.UPDATED_AT)
                    .values(ledgerItems.getId(), ledgerItems.getLedgerId(), ledgerItems.getCoa(),
                            ledgerItems.getAmount(), ledgerItems.getType(), ledgerItems.getCreatedAt(),
                            ledgerItems.getUpdatedAt())
                    .execute();
        } catch (Exception e) {
    ...
    public void deleteLedgerItemsByLedgerId(UUID ledgerId) {
        try {
            dslContext.delete(Tables.LEDGER_ITEMS)
                    .where(Tables.LEDGER_ITEMS.LEDGER_ID.eq(ledgerId))
                    .execute();
        } catch (Exception e) {
            log.error("Error deleting ledger items by ledger ID", e);
            throw new JooqOperationException("Failed to delete LedgerItems by Ledger ID");
        }
    }
